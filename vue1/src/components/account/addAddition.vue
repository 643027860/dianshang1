<template>
  <div class="addAddition">
    ...
    <p>
      ...
      <el-button @click="saveAddition">确定</el-button>
    </p>
  </div>
</template>

<script>
  import {ADDADDITION,REQADDITIONS} from '../../api/types'
  ...
  export default {
    ...
    methods: {
      ...mapActions(['ocaddaddition','saveadditions']),
      //手机验证
      rulePhone(){
      ...
      async saveAddition(){
        ...
        var {phoneOk,addition,receiverName,receiverAddress,receiverZip,receiverMobile,receiverPhone} = this
        if(addition,receiverName,receiverAddress,receiverZip,receiverMobile,receiverPhone){
          if(!phoneOk){
          ...
          var result = await ADDADDITION(params)
          if(result.status ==0 ){
            this.message('新增地址成功',1000,'success')
            this.pageNum = 1
